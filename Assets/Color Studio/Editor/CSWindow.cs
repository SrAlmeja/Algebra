...
namespace ColorStudio {
    ...
    public partial class CSWindow : EditorWindow {
        ...
        [SerializeField] string selectionHex;
        [SerializeField] string selectionRGB;
        [SerializeField] string selectionRGB01;
        [SerializeField] string selectionHSL;
        ...
        void OnGUI() {
            ...
            if (viewMode == ViewMode.ColorWheel || viewMode == ViewMode.Palette || viewMode == ViewMode.Tools) {
                ...
                if (viewMode != ViewMode.Tools) {
                    ...
                    if (selectionColor.a != 0) {
                        ...
                        EditorGUILayout.EndHorizontal();
                        EditorGUI.DrawRect(space, selectionColor);
                        var style = new GUIStyle();
                        style.alignment = TextAnchor.MiddleCenter;
                        style.normal.textColor = selectionColor.g > 0.5f ? Color.black : Color.white;
                        style.fontStyle = FontStyle.Bold;
                        EditorGUI.SelectableLabel(space, selectionHex, style);
                    }
                    EditorGUILayout.BeginHorizontal();
                    selectionRGB01 = EditorGUILayout.TextField("RGB 0-1", selectionRGB01);
                    GUI.enabled = !string.IsNullOrEmpty(selectionRGB01);
                    ...
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    selectionRGB = EditorGUILayout.TextField("RGB 0-255", selectionRGB);
                    GUI.enabled = !string.IsNullOrEmpty(selectionRGB);
                    if (GUILayout.Button("Add", EditorStyles.miniButtonRight, GUILayout.Width(50))) {
                        AddRGB();
                    }
                    GUI.enabled = true;
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    selectionHSL = EditorGUILayout.TextField("HSL", selectionHSL);
                    GUI.enabled = !string.IsNullOrEmpty(selectionHSL);
                    if (GUILayout.Button("Add", EditorStyles.miniButtonRight, GUILayout.Width(50))) {
                        AddHSL();
                    }
                    GUI.enabled = true;
                ...
                if (palettePos.x >= 0 && palettePos.x <= 1f && palettePos.y >= 0 && palettePos.y <= 1f) {
                    ...
                    if (mouseDown) {
                        ...
                        float L = Mathf.Clamp01((Mathf.FloorToInt(y * palette.shades) + 0.5f) / palette.shades);
                        if (palette.scheme == ColorScheme.Custom) {
                            ...
                            selectionColor = ColorConversion.GetColorFromRGBSL(palette.colors[colorIndex].r, palette.colors[colorIndex].g, palette.colors[colorIndex].b, palette.saturation, L);
                        }
                        if (click) {
                        ...
                        selectionRGB01 = selectionColor.r.ToString("F3", CultureInfo.InvariantCulture) + ", " + selectionColor.g.ToString("F3", CultureInfo.InvariantCulture) + ", " + selectionColor.b.ToString("F3", CultureInfo.InvariantCulture);
                        selectionRGB = ((int)(selectionColor.r * 255)).ToString(CultureInfo.InvariantCulture) + ", " + ((int)(selectionColor.g * 255)).ToString(CultureInfo.InvariantCulture) + ", " + ((int)(selectionColor.b * 255)).ToString(CultureInfo.InvariantCulture);
                        selectionHex = ((int)(selectionColor.r * 255)).ToString("X2") + ((int)(selectionColor.g * 255)).ToString("X2") + ((int)(selectionColor.b * 255)).ToString("X2");

                        float hue = ColorConversion.GetHue(selectionColor.r, selectionColor.g, selectionColor.b);
                        selectionHSL = hue.ToString("F3", CultureInfo.InvariantCulture) + ", " + palette.saturation.ToString("F3", CultureInfo.InvariantCulture) + ", " + L.ToString("F3", CultureInfo.InvariantCulture);
                    }
                }
                EditorGUILayout.EndVertical();
            ...
            if (viewMode == ViewMode.Palette) {
                ...
                if (GUILayout.Button(new GUIContent("Save", "Replace existing palette file or create a new palette file."), GUILayout.Width(width))) {
                    if (otherPalette != null) {
        ...
        void SetColorKeys(CSPalette palette) {
            selectionPos.x = -1;
            selectionHex = selectionRGB = selectionRGB01 = selectionHSL = "";
            selectedCustomColor = -1;
            selectionColor.a = 0;
        ...
        void AddRGB01() {
            float[] rgb = StringTo3Floats(selectionRGB01);
            if (rgb != null) {
        ...
        void AddRGB() {
            float[] rgb = StringTo3Floats(selectionRGB);
            if (rgb != null) {
        ...
        void AddHSL() {
            float[] hsl = StringTo3Floats(selectionHSL);
            if (hsl != null) {
